...
class _CUDADriverLibrary:
    ...
    def _bind_api(self) -> None:
        ...
        self._cuLaunchKernel = self._symbol("cuLaunchKernel")
        self._cuLaunchKernel.argtypes = [
            ctypes.c_void_p,
            ctypes.c_uint,
            ctypes.c_uint,
            ctypes.c_uint,
        ...
        self._cuLaunchKernel.restype = ctypes.c_int

        self._cuCtxSynchronize = self._symbol("cuCtxSynchronize")
        self._cuCtxSynchronize.argtypes = []
        self._cuCtxSynchronize.restype = ctypes.c_int
    ...
    def launch_kernel(
        ...
        *,
        ...
        kernel_params: Iterable[int],
        shared_mem_bytes: int = 0,
        stream: int = 0,
    ) -> None:
        """Launch one CUfunction with pointer-valued kernel arguments.

        The kernels generated through v0.20 take only device-pointer
        arguments. cuLaunchKernel expects ``void** kernelParams`` where each
        entry points at host storage containing one argument value, so keep a
        c_uint64 object alive for every CUdeviceptr until the call returns.
        """
        ...
        if len(grid) != 3 or any(value <= 0 for value in grid):
            raise ValueError(f"invalid CUDA grid dimensions: {grid}")
        if len(block) != 3 or any(value <= 0 for value in block):
            raise ValueError(f"invalid CUDA block dimensions: {block}")

        argument_storage = [
            _CUdeviceptr(int(pointer))
            for pointer in kernel_params
        ]
        param_array_type = ctypes.c_void_p * len(argument_storage)
        param_array = param_array_type(
            *(
                ctypes.cast(ctypes.pointer(value), ctypes.c_void_p)
                for value in argument_storage
            )
        )

        self._check(
            self._cuLaunchKernel(
                ctypes.c_void_p(int(function)),
                grid[0],
                grid[1],
                grid[2],
                block[0],
