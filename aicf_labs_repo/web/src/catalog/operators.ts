...
import { OperatorMask, type Operator, type OperatorId } from '../domain/operator';
import { PropertyKind, type PropertyClaim } from '../domain/property';
import {
...
function whole(kind: PropertyClaim['kind'], justification?: string): PropertyClaim {
  return { kind, scope: { kind: 'operator' }, conditions: [], ...(justification ? { justification } : {}) };
}

const pure = whole(PropertyKind.PURE, 'The operator has no observable side effects in the browser IR.');
const elementwise = whole(
  PropertyKind.ELEMENTWISE,
  'Each output element depends only on corresponding or broadcast-compatible input elements, with no cross-element dependency.',
);
const reduction = whole(
  PropertyKind.REDUCTION,
  'The operator combines elements along explicitly configured reduction axes into one or fewer output elements.',
);

function unknownSemantics(reason: string): OperatorSemanticFacts {
  return {
    scope: 'operator',
    dependencyFootprint: { kind: DependencyFootprintKind.UNKNOWN, reason },
    reuse: { kind: ReuseKind.UNKNOWN, reason: 'Reuse is not modeled for this operator.' },
  };
}

const exactElementSemantics: OperatorSemanticFacts = {
  scope: 'operator',
  dependencyFootprint: {
    kind: DependencyFootprintKind.CORRESPONDING_ELEMENT,
    inputMapping: CorrespondingElementMapping.EXACT,
    reason: 'Each output unit reads only the input unit at the same logical index.',
  },
  reuse: { kind: ReuseKind.NONE, reason: 'An input element contributes to one corresponding output element.' },
};

const broadcastElementSemantics: OperatorSemanticFacts = {
  scope: 'operator',
  dependencyFootprint: {
    kind: DependencyFootprintKind.CORRESPONDING_ELEMENT,
    inputMapping: CorrespondingElementMapping.EXACT_OR_SCALAR_BROADCAST,
    reason: 'Each output unit reads corresponding elements; scalar broadcast is explicitly resolvable.',
  },
  reuse: { kind: ReuseKind.UNKNOWN, reason: 'Broadcast shape decides whether an input value is reused across outputs.' },
};

const reductionSumSemantics: OperatorSemanticFacts = {
  scope: 'operator',
  dependencyFootprint: {
    kind: DependencyFootprintKind.FULL_AXIS,
    axis: 'FROM_OPERATOR_ATTRIBUTE',
    reason: 'Each output unit depends on every input element along the configured reduction axis.',
  },
  reuse: { kind: ReuseKind.WITHIN_OUTPUT, reason: 'Many input elements contribute to one reduction output state.' },
  partialState: {
...
export const OPERATORS: readonly Operator[] = [
  {
    id: 'input', name: 'Input', symbol: 'x', category: 'source', arity: 0, mask: OperatorMask.NONE, propertyClaims: [], semanticFacts: unknownSemantics('Input is a graph boundary and has no producer-input dependency footprint.'), inputPorts: [],
  ...
  {
    id: 'constant', name: 'Constant', symbol: 'c', category: 'source', arity: 0, mask: OperatorMask.NONE, propertyClaims: [], semanticFacts: unknownSemantics('Constant has no producer-input dependency footprint.'), inputPorts: [],
  ...
  },
  {
    ...
    mask: OperatorMask.ELEMENTWISE | OperatorMask.COMMUTATIVE | OperatorMask.PURE | OperatorMask.BROADCAST,
    propertyClaims: [
      pure,
      elementwise,
      whole(PropertyKind.COMMUTATIVE, 'a + b = b + a over abstract real-number semantics.'),
      whole(PropertyKind.ASSOCIATIVE, '(a + b) + c = a + (b + c) over abstract real-number semantics.'),
    ],
    semanticFacts: broadcastElementSemantics,
    ...
    applicableRewriteRuleIds: ['add-zero', 'add-commute', 'constant-fold', 'reassociate-associative-right', 'embed-elementwise-producer-into-reduction'],
  ...
  },
  {
    ...
    applicableRewriteRuleIds: ['mul-one', 'mul-zero', 'mul-commute', 'constant-fold', 'reassociate-associative-right', 'embed-elementwise-producer-into-reduction'],
  ...
  {
    id: 'matmul', name: 'MatMul', symbol: '@', category: 'matrix', arity: 2, mask: OperatorMask.PURE,
    propertyClaims: [
      pure,
      {
        kind: PropertyKind.LINEAR,
        scope: { kind: 'input', inputPort: 'in-0' },
        conditions: [{ kind: 'input-fixed', inputPort: 'in-1' }],
        justification: 'MatMul is linear in input 0 while input 1 is held fixed.',
      },
      {
        kind: PropertyKind.LINEAR,
        scope: { kind: 'input', inputPort: 'in-1' },
        conditions: [{ kind: 'input-fixed', inputPort: 'in-0' }],
        justification: 'MatMul is linear in input 1 while input 0 is held fixed.',
      },
    ],
    semanticFacts: unknownSemantics('MatMul row/column dependency regions are not modeled by the current footprint vocabulary.'),
    inputPorts: [{ id: 'in-0', label: 'left', required: true }, { id: 'in-1', label: 'right', required: true }],
  ...
  },
  {
    ...
    mask: OperatorMask.ELEMENTWISE | OperatorMask.PURE | OperatorMask.SHAPE_PRESERVING,
    propertyClaims: [
      pure,
      elementwise,
      whole(PropertyKind.SHAPE_PRESERVING, 'Each ReLU output element has the same index and shape as its input element.'),
      {
        kind: PropertyKind.POSITIVE_HOMOGENEOUS,
        scope: { kind: 'input', inputPort: 'in-0' },
        conditions: [{ kind: 'scale-comparison', operator: '>=', value: 0 }],
        justification: 'ReLU(alpha × x) = alpha × ReLU(x) when alpha >= 0.',
      },
    ],
    semanticFacts: exactElementSemantics,
    ...
    algebraicProperties: ['멱등: ReLU(ReLU(x)) = ReLU(x)'], numericalNotes: ['NaN 처리 방식은 구체 max 구현에 따라 달라질 수 있습니다.'], applicableRewriteRuleIds: ['constant-fold', 'embed-elementwise-producer-into-reduction'],
  ...
  },
  {
    ...
    mask: OperatorMask.PURE | OperatorMask.PERMUTATION,
    propertyClaims: [pure, whole(PropertyKind.PERMUTATION, 'Transpose permutes the final two axes without changing values.')],
    semanticFacts: unknownSemantics('Permutation-index dependency mapping is not modeled by the current footprint vocabulary.'),
  ...
  },
  {
    ...
    mask: OperatorMask.REDUCTION | OperatorMask.PURE,
    propertyClaims: [
      pure,
      reduction,
      {
        ...
        scope: { kind: 'input', inputPort: 'in-0' },
        conditions: [],
        justification: 'ReduceSum(alpha × x) = alpha × ReduceSum(x) over abstract real-number semantics.',
      },
    ],
    semanticFacts: reductionSumSemantics,
    ...
    algebraicProperties: ['선형 연산'], numericalNotes: ['합산 순서에 따라 부동소수점 결과가 달라질 수 있습니다.'], applicableRewriteRuleIds: ['embed-elementwise-producer-into-reduction'],
