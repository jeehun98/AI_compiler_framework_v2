...
__global__ void max_fp32(const float* a, const float* b, float* output,
                            int element_count) {
  const int index = blockIdx.x * blockDim.x + threadIdx.x;
  if (index < element_count) {
    output[index] = fmaxf(a[index], b[index]);
  }
}
...
float benchmark_us(const float* a, const float* b, float* output,
  ...
  for (int i = 0; i < warmup_iterations; ++i) {
    max_fp32<<<blocks, kThreads>>>(a, b, output, element_count);
  }
...
void validate(int element_count, std::uint32_t seed) {
  ...
  max_fp32<<<blocks_for(element_count), kThreads>>>(
  ...
  for (int i = 0; i < element_count; ++i) {
    const float reference = std::fmax(inputs.a[i], inputs.b[i]);
  ...
  if (tolerance_violations != 0) {
    throw std::runtime_error("FP32 max validation failed");
  }
  ...
  try {
    if (argc > 5) {
      throw std::invalid_argument(
          "usage: max.exe [elements] [iterations] [validation_elements] "
          "[seed]");
    }
    ...
    std::cout << "Elementwise max: " << elapsed_us << " us\n";
