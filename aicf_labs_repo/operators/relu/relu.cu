...
__global__ void relu_fp32(const float* input, float* output,
                          int element_count) {
...
float benchmark_us(const float* input, float* output, int element_count,
  ...
  for (int i = 0; i < kWarmupIterations; ++i) {
    relu_fp32<<<blocks, kThreads>>>(input, output, element_count);
  }
...
void validate(int element_count, std::uint32_t seed) {
  ...
  relu_fp32<<<blocks_for(element_count), kThreads>>>(
  ...
  for (int i = 0; i < element_count; ++i) {
    const float reference = std::fmax(input[i], 0.0F);
  ...
  if (mismatches != 0) {
    throw std::runtime_error("FP32 ReLU validation failed");
  }
  ...
  try {
    if (argc > 5) {
      throw std::invalid_argument(
          "usage: relu.exe [elements] [iterations] [validation_elements] "
          "[seed]");
    }
    ...
    std::cout << "Elementwise ReLU: " << elapsed_us << " us\n";
