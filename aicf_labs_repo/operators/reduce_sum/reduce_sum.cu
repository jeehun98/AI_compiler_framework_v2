...
__global__ void reduce_sum_fp32(const float* input, float* output,
                                int element_count) {
...
float benchmark_us(const float* input, float* output, int element_count,
  ...
  for (int i = 0; i < kWarmupIterations; ++i) {
    ...
    reduce_sum_fp32<<<blocks, kThreads>>>(input, output, element_count);
  }
...
void validate(int element_count, int multiprocessor_count, std::uint32_t seed) {
  ...
  CUDA_CHECK(cudaMemset(device_output.get(), 0, sizeof(float)));

  const int blocks = reduction_blocks(element_count, multiprocessor_count);
  reduce_sum_fp32<<<blocks, kThreads>>>(
  ...
  if (!passed) {
    throw std::runtime_error("FP32 reduce_sum validation failed");
  }
  ...
  try {
    if (argc > 5) {
      throw std::invalid_argument(
          "usage: reduce_sum.exe [elements] [iterations] "
    ...
    std::cout << "Reduce sum: " << elapsed_us << " us\n";
