...
__global__ void sigmoid_fp32(const float* input, float* output,
                             int element_count) {
  const int index = blockIdx.x * blockDim.x + threadIdx.x;
  if (index < element_count) {
    const float exponent = expf(-input[index]);
    output[index] = 1.0F / (1.0F + exponent);
  }
}
...
float benchmark_us(const float* input, float* output, int element_count,
  ...
  for (int i = 0; i < warmup_iterations; ++i) {
    sigmoid_fp32<<<blocks, kThreads>>>(input, output, element_count);
  }
...
void validate(int element_count, std::uint32_t seed) {
  ...
  sigmoid_fp32<<<blocks_for(element_count), kThreads>>>(
  ...
  for (int i = 0; i < element_count; ++i) {
    const double input_value = static_cast<double>(input[i]);
  ...
  if (tolerance_violations != 0) {
    throw std::runtime_error("FP32 sigmoid validation failed");
  }
  ...
  try {
    if (argc > 5) {
      throw std::invalid_argument(
          "usage: sigmoid.exe [elements] [iterations] [validation_elements] "
          "[seed]");
    }
    ...
    std::cout << "Elementwise sigmoid: " << elapsed_us << " us\n";
