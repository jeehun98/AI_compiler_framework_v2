...
__global__ void div_fp32(const float* numerator, const float* denominator,
                         float* output, int element_count) {
...
struct HostInputs {
  explicit HostInputs(std::size_t element_count)
      : numerator(element_count), denominator(element_count) {}
...
HostInputs make_inputs(std::size_t element_count, std::uint32_t seed) {
  ...
  for (std::size_t i = 0; i < element_count; ++i) {
    inputs.numerator[i] = distribution(generator);
    float denominator = distribution(generator);
    if (std::abs(denominator) < 1.0F) {
      denominator = std::copysign(1.0F, denominator == 0.0F ? 1.0F
                                                            : denominator);
    }
    inputs.denominator[i] = denominator;
  }
  return inputs;
}
...
void copy_inputs_to_device(const HostInputs& inputs,
                           DeviceFloatBuffer& numerator,
                           DeviceFloatBuffer& denominator) {
  const std::size_t bytes = inputs.numerator.size() * sizeof(float);
  CUDA_CHECK(cudaMemcpy(numerator.get(), inputs.numerator.data(), bytes,
                        cudaMemcpyHostToDevice));
  CUDA_CHECK(cudaMemcpy(denominator.get(), inputs.denominator.data(), bytes,
                        cudaMemcpyHostToDevice));
}

float benchmark_us(const float* numerator, const float* denominator,
  ...
  for (int i = 0; i < warmup_iterations; ++i) {
    div_fp32<<<blocks, kThreads>>>(numerator, denominator, output,
                                  element_count);
  }
...
void validate(int element_count, std::uint32_t seed) {
  ...
  DeviceFloatBuffer numerator(element_count);
  DeviceFloatBuffer denominator(element_count);
  DeviceFloatBuffer output(element_count);
  copy_inputs_to_device(inputs, numerator, denominator);

  div_fp32<<<blocks_for(element_count), kThreads>>>(
      numerator.get(), denominator.get(), output.get(), element_count);
  CUDA_CHECK(cudaGetLastError());
  CUDA_CHECK(cudaDeviceSynchronize());
  ...
  for (int i = 0; i < element_count; ++i) {
    const float reference = inputs.numerator[i] / inputs.denominator[i];
  ...
  if (tolerance_violations != 0) {
    throw std::runtime_error("FP32 div validation failed");
  }
  ...
  try {
    if (argc > 5) {
      throw std::invalid_argument(
          "usage: div.exe [elements] [iterations] [validation_elements] "
          "[seed]");
    }
    ...
    DeviceFloatBuffer numerator(elements);
    DeviceFloatBuffer denominator(elements);
    DeviceFloatBuffer output(elements);
    copy_inputs_to_device(inputs, numerator, denominator);

    const float elapsed_us = benchmark_us(numerator.get(), denominator.get(),
    ...
    std::cout << "Elementwise div: " << elapsed_us << " us\n";
