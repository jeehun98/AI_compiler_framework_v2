...
__global__ void reduce_mean_sum_fp32(const float* input, float* output,
                                int element_count) {
...
__global__ void scale_mean_fp32(float* output, int element_count) {
  if (blockIdx.x == 0 && threadIdx.x == 0) {
    output[0] *= 1.0F / static_cast<float>(element_count);
  }
}
...
void launch_reduce_mean(const float* input, float* output, int element_count,
                        int blocks) {
  CUDA_CHECK(cudaMemsetAsync(output, 0, sizeof(float)));
  reduce_mean_sum_fp32<<<blocks, kThreads>>>(input, output, element_count);
  scale_mean_fp32<<<1, 1>>>(output, element_count);
}
...
float benchmark_us(const float* input, float* output, int element_count,
  ...
  for (int i = 0; i < warmup_iterations; ++i) {
    launch_reduce_mean(input, output, element_count, blocks);
  }
...
void validate(int element_count, int multiprocessor_count, std::uint32_t seed) {
  ...
  launch_reduce_mean(device_input.get(), device_output.get(), element_count,
                     blocks);
  CUDA_CHECK(cudaGetLastError());
  CUDA_CHECK(cudaDeviceSynchronize());
  ...
  reference /= static_cast<double>(element_count);
  const double absolute_error =
      std::abs(static_cast<double>(gpu_output) - reference);
  const double relative_error =
      reference != 0.0 ? absolute_error / std::abs(reference) : absolute_error;
  const double tolerance =
      kAbsoluteTolerance + kRelativeTolerance * std::abs(reference);
  const bool passed = absolute_error <= tolerance;
  ...
  if (!passed) {
    throw std::runtime_error("FP32 reduce_mean validation failed");
  }
  ...
  try {
    if (argc > 5) {
      throw std::invalid_argument(
          "usage: reduce_mean.exe [elements] [iterations] "
    ...
    std::cout << "Reduce mean: " << elapsed_us << " us\n";
