...
constexpr float kLowerBound = -2.0F;
constexpr float kUpperBound = 2.0F;
...
__global__ void clamp_fp32(const float* input, float* output,
                            int element_count) {
  const int index = blockIdx.x * blockDim.x + threadIdx.x;
  if (index < element_count) {
    output[index] =
        fminf(fmaxf(input[index], kLowerBound), kUpperBound);
  }
}
...
float benchmark_us(const float* input, float* output, int element_count,
  ...
  for (int i = 0; i < warmup_iterations; ++i) {
    clamp_fp32<<<blocks, kThreads>>>(input, output, element_count);
  }
...
void validate(int element_count, std::uint32_t seed) {
  ...
  clamp_fp32<<<blocks_for(element_count), kThreads>>>(
  ...
  for (int i = 0; i < element_count; ++i) {
    const float reference =
        std::min(std::max(input[i], kLowerBound), kUpperBound);
  ...
  if (tolerance_violations != 0) {
    throw std::runtime_error("FP32 clamp validation failed");
  }
  ...
  try {
    if (argc > 5) {
      throw std::invalid_argument(
          "usage: clamp.exe [elements] [iterations] [validation_elements] "
          "[seed]");
    }
    ...
    std::cout << "Elementwise clamp: " << elapsed_us << " us\n";
