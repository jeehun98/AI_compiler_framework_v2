...
__global__ void sqrt_fp32(const float* input, float* output,
                            int element_count) {
...
float benchmark_us(const float* input, float* output, int element_count,
  ...
  for (int i = 0; i < warmup_iterations; ++i) {
    sqrt_fp32<<<blocks, kThreads>>>(input, output, element_count);
  }
...
void validate(int element_count, std::uint32_t seed) {
  ...
  sqrt_fp32<<<blocks_for(element_count), kThreads>>>(
  ...
  for (int i = 0; i < element_count; ++i) {
    const float reference = static_cast<float>(std::sqrt(static_cast<double>(input[i])));
  ...
  if (tolerance_violations != 0) {
    throw std::runtime_error("FP32 sqrt validation failed");
  }
  ...
  try {
    if (argc > 5) {
      throw std::invalid_argument(
          "usage: sqrt.exe [elements] [iterations] [validation_elements] "
          "[seed]");
    }
    ...
    std::cout << "Elementwise sqrt: " << elapsed_us << " us\n";
