...
constexpr float kAbsoluteTolerance = 0.0F;
constexpr float kRelativeTolerance = 0.0F;
...
__global__ void where_fp32(const float* condition, const float* a,
...
struct HostInputs {
  explicit HostInputs(std::size_t element_count)
      : condition(element_count), a(element_count), b(element_count) {}
...
HostInputs make_inputs(std::size_t element_count, std::uint32_t seed) {
  HostInputs inputs(element_count);
  std::mt19937 generator(seed);
  std::uniform_real_distribution<float> distribution(-1000.0F, 1000.0F);
  for (std::size_t i = 0; i < element_count; ++i) {
    inputs.condition[i] = (i & 1U) != 0U ? 1.0F : 0.0F;
    inputs.a[i] = distribution(generator);
    inputs.b[i] = distribution(generator);
  }
  return inputs;
}
...
void copy_inputs_to_device(const HostInputs& inputs,
                           DeviceFloatBuffer& condition, DeviceFloatBuffer& a,
                           DeviceFloatBuffer& b) {
  const std::size_t bytes = inputs.a.size() * sizeof(float);
  CUDA_CHECK(cudaMemcpy(condition.get(), inputs.condition.data(), bytes,
                        cudaMemcpyHostToDevice));
  CUDA_CHECK(
      cudaMemcpy(a.get(), inputs.a.data(), bytes, cudaMemcpyHostToDevice));
  CUDA_CHECK(
      cudaMemcpy(b.get(), inputs.b.data(), bytes, cudaMemcpyHostToDevice));
}

float benchmark_us(const float* condition, const float* a, const float* b,
                   float* output, int element_count, int iterations) {
  const int warmup_iterations = iterations > 1 ? std::min(10, iterations) : 0;
  const int blocks = blocks_for(element_count);
  for (int i = 0; i < warmup_iterations; ++i) {
    where_fp32<<<blocks, kThreads>>>(condition, a, b, output, element_count);
  }
...
void validate(int element_count, std::uint32_t seed) {
  const HostInputs inputs = make_inputs(element_count, seed);
  const std::size_t bytes =
      static_cast<std::size_t>(element_count) * sizeof(float);
  DeviceFloatBuffer condition(element_count);
  DeviceFloatBuffer a(element_count);
  DeviceFloatBuffer b(element_count);
  DeviceFloatBuffer output(element_count);
  copy_inputs_to_device(inputs, condition, a, b);

  where_fp32<<<blocks_for(element_count), kThreads>>>(
      condition.get(), a.get(), b.get(), output.get(), element_count);
  CUDA_CHECK(cudaGetLastError());
  CUDA_CHECK(cudaDeviceSynchronize());

  std::vector<float> gpu_output(element_count);
  CUDA_CHECK(cudaMemcpy(gpu_output.data(), output.get(), bytes,
  ...
  for (int i = 0; i < element_count; ++i) {
    const float reference = inputs.condition[i] != 0.0F ? inputs.a[i]
                                                           : inputs.b[i];
  ...
  if (tolerance_violations != 0) {
    throw std::runtime_error("FP32 where validation failed");
  }
  ...
  try {
    if (argc > 5) {
      throw std::invalid_argument(
          "usage: where.exe [elements] [iterations] [validation_elements] "
          "[seed]");
    }
    ...
    const HostInputs inputs = make_inputs(elements, seed);
    const std::size_t bytes = static_cast<std::size_t>(elements) * sizeof(float);
    DeviceFloatBuffer condition(elements);
    DeviceFloatBuffer a(elements);
    DeviceFloatBuffer b(elements);
    DeviceFloatBuffer output(elements);
    copy_inputs_to_device(inputs, condition, a, b);

    const float elapsed_us = benchmark_us(condition.get(), a.get(), b.get(),
                                          output.get(), elements, iterations);
    const double transferred_bytes = 4.0 * static_cast<double>(bytes);
    ...
    std::cout << "Elementwise where: " << elapsed_us << " us\n";
