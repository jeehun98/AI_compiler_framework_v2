...
__global__ void neg_fp32(const float* input, float* output,
                            int element_count) {
...
float benchmark_us(const float* input, float* output, int element_count,
  ...
  for (int i = 0; i < warmup_iterations; ++i) {
    neg_fp32<<<blocks, kThreads>>>(input, output, element_count);
  }
...
void validate(int element_count, std::uint32_t seed) {
  ...
  neg_fp32<<<blocks_for(element_count), kThreads>>>(
  ...
  if (tolerance_violations != 0) {
    throw std::runtime_error("FP32 neg validation failed");
  }
  ...
  try {
    if (argc > 5) {
      throw std::invalid_argument(
          "usage: neg.exe [elements] [iterations] [validation_elements] "
          "[seed]");
    }
    ...
    std::cout << "Elementwise neg: " << elapsed_us << " us\n";
