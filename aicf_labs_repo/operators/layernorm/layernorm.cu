...
__global__ void layernorm_fp32(const float* input, const float* gamma,
                               const float* beta, float* output, int cols) {
  ...
  for (int col = threadIdx.x; col < cols; col += blockDim.x) {
    const float difference = input[row_offset + col] - shared_mean;
    thread_variance =
        fmaf(difference, difference, thread_variance);
  }
  thread_variance = warp_reduce_sum(thread_variance);
  if (lane == 0) {
    warp_values[warp] = thread_variance;
  }
  __syncthreads();
  ...
  if (warp == 0) {
    ...
    block_variance = warp_reduce_sum(block_variance);
    if (lane == 0) {
      const float variance =
          block_variance / static_cast<float>(cols);
      shared_inv_std = rsqrtf(variance + kEpsilon);
    }
  }
  __syncthreads();
  ...
  for (int col = threadIdx.x; col < cols; col += blockDim.x) {
    const float normalized =
        (input[row_offset + col] - shared_mean) * shared_inv_std;
    output[row_offset + col] =
        fmaf(normalized, gamma[col], beta[col]);
  }
}
...
struct HostData {
  HostData(std::size_t element_count, std::size_t column_count)
      : input(element_count), gamma(column_count), beta(column_count) {}
...
HostData make_data(int rows, int cols, std::uint32_t seed) {
  ...
  std::uniform_real_distribution<float> beta_distribution(-0.5F, 0.5F);
  for (float& value : data.input) {
    value = input_distribution(generator);
  }
  for (int col = 0; col < cols; ++col) {
    data.gamma[col] = gamma_distribution(generator);
    data.beta[col] = beta_distribution(generator);
  }
  return data;
}
...
void copy_data_to_device(const HostData& data, DeviceFloatBuffer& input,
  ...
  CUDA_CHECK(cudaMemcpy(gamma.get(), data.gamma.data(),
                        ...
                        cudaMemcpyHostToDevice));
  CUDA_CHECK(cudaMemcpy(beta.get(), data.beta.data(),
                        data.beta.size() * sizeof(float),
                        cudaMemcpyHostToDevice));
}

float benchmark_us(const float* input, const float* gamma, const float* beta,
                   float* output, int rows, int cols, int iterations) {
  const int warmup_iterations = iterations > 1 ? std::min(10, iterations) : 0;
  for (int i = 0; i < warmup_iterations; ++i) {
    layernorm_fp32<<<rows, kThreads>>>(input, gamma, beta, output, cols);
  }
...
void validate(int rows, int cols, std::uint32_t seed) {
  ...
  DeviceFloatBuffer device_beta(static_cast<std::size_t>(cols));
  DeviceFloatBuffer device_output(element_count);
  copy_data_to_device(data, device_input, device_gamma, device_beta);

  layernorm_fp32<<<rows, kThreads>>>(
      device_input.get(), device_gamma.get(), device_beta.get(),
  ...
  for (int row = 0; row < rows; ++row) {
    ...
    double mean = 0.0;
    for (int col = 0; col < cols; ++col) {
      mean += static_cast<double>(data.input[row_offset + col]);
    }
    ...
    for (int col = 0; col < cols; ++col) {
      const double difference =
          static_cast<double>(data.input[row_offset + col]) - mean;
      variance += difference * difference;
    }
    variance /= static_cast<double>(cols);
    const double inv_std =
        1.0 / std::sqrt(variance + static_cast<double>(kEpsilon));

    for (int col = 0; col < cols; ++col) {
      const std::size_t index = row_offset + static_cast<std::size_t>(col);
      const double normalized =
          (static_cast<double>(data.input[index]) - mean) * inv_std;
      const float reference = static_cast<float>(
          normalized * static_cast<double>(data.gamma[col]) +
          static_cast<double>(data.beta[col]));
  ...
  if (tolerance_violations != 0 || non_finite_outputs != 0) {
    throw std::runtime_error("FP32 layernorm validation failed");
  }
  ...
  try {
    if (argc > 6) {
      throw std::invalid_argument(
          "usage: layernorm.exe [rows] [cols] [iterations] "
    ...
    const float elapsed_us = benchmark_us(
        device_input.get(), device_gamma.get(), device_beta.get(),
        device_output.get(), rows, cols, iterations);
    ...
    std::cout << "Row-wise layernorm: " << elapsed_us << " us\n";
