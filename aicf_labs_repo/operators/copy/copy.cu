...
__global__ void copy_fp32(const float* input, float* output,
                          int element_count) {
...
float benchmark_us(const float* input, float* output, int element_count,
  ...
  for (int i = 0; i < kWarmupIterations; ++i) {
    copy_fp32<<<blocks, kThreads>>>(input, output, element_count);
  }
...
void validate(int element_count, std::uint32_t seed) {
  ...
  copy_fp32<<<blocks_for(element_count), kThreads>>>(
  ...
  if (mismatches != 0) {
    throw std::runtime_error("FP32 copy validation failed");
  }
  ...
  try {
    if (argc > 5) {
      throw std::invalid_argument(
          "usage: copy.exe [elements] [iterations] [validation_elements] "
          "[seed]");
    }
    ...
    std::cout << "Elementwise copy: " << elapsed_us << " us\n";
