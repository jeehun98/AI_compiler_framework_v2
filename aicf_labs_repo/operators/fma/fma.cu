...
ValidationStats run_validation_case(const HostInputs& inputs,
  ...
  const int pair_count = static_cast<int>(element_count / 2);
  const int blocks = (pair_count + kThreads - 1) / kThreads;
  mul_half2<<<blocks, kThreads>>>(device_a.get(), device_b.get(),
                                  temporary.get(), pair_count);
  add_half2<<<blocks, kThreads>>>(temporary.get(), device_c.get(),
                                  separated_output.get(), pair_count);
  fma_half2<<<blocks, kThreads>>>(device_a.get(), device_b.get(),
                                  device_c.get(), fused_output.get(),
                                  pair_count);
  CUDA_CHECK(cudaGetLastError());
  CUDA_CHECK(cudaDeviceSynchronize());
