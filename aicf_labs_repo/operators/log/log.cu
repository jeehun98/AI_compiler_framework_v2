...
__global__ void log_fp32(const float* input, float* output,
                            int element_count) {
...
std::vector<float> make_input(std::size_t element_count, std::uint32_t seed) {
  ...
  std::uniform_real_distribution<float> distribution(1.0e-4F, 1000.0F);
...
float benchmark_us(const float* input, float* output, int element_count,
  ...
  for (int i = 0; i < warmup_iterations; ++i) {
    log_fp32<<<blocks, kThreads>>>(input, output, element_count);
  }
...
void validate(int element_count, std::uint32_t seed) {
  ...
  log_fp32<<<blocks_for(element_count), kThreads>>>(
  ...
  for (int i = 0; i < element_count; ++i) {
    const float reference = static_cast<float>(std::log(static_cast<double>(input[i])));
  ...
  if (tolerance_violations != 0) {
    throw std::runtime_error("FP32 log validation failed");
  }
  ...
  try {
    if (argc > 5) {
      throw std::invalid_argument(
          "usage: log.exe [elements] [iterations] [validation_elements] "
          "[seed]");
    }
    ...
    std::cout << "Elementwise log: " << elapsed_us << " us\n";
