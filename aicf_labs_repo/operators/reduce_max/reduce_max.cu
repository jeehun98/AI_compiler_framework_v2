...
__device__ __forceinline__ void atomic_max_fp32(float* address, float value) {
...
__global__ void initialize_max_fp32(float* output) {
  output[0] = -CUDART_INF_F;
}

__global__ void reduce_max_fp32(const float* input, float* output,
                                int element_count) {
  ...
  if (lane == 0) {
    warp_maxima[warp] = thread_max;
  }
  __syncthreads();

  if (warp == 0) {
    float block_max =
        lane < kWarpsPerBlock ? warp_maxima[lane] : -CUDART_INF_F;
    block_max = warp_reduce_max(block_max);
    if (lane == 0) {
      atomic_max_fp32(output, block_max);
    }
  }
}
...
void launch_reduce_max(const float* input, float* output, int element_count,
                       int blocks) {
  initialize_max_fp32<<<1, 1>>>(output);
  reduce_max_fp32<<<blocks, kThreads>>>(input, output, element_count);
}
...
float benchmark_us(const float* input, float* output, int element_count,
  ...
  for (int i = 0; i < warmup_iterations; ++i) {
    launch_reduce_max(input, output, element_count, blocks);
  }
...
void validate(int element_count, int multiprocessor_count, std::uint32_t seed) {
  ...
  launch_reduce_max(device_input.get(), device_output.get(), element_count,
                    blocks);
  CUDA_CHECK(cudaGetLastError());
  CUDA_CHECK(cudaDeviceSynchronize());
  ...
  if (!passed) {
    throw std::runtime_error("FP32 reduce_max validation failed");
  }
  ...
  try {
    if (argc > 5) {
      throw std::invalid_argument(
          "usage: reduce_max.exe [elements] [iterations] "
    ...
    std::cout << "Reduce max: " << elapsed_us << " us\n";
