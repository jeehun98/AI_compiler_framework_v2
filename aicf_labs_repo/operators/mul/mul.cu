...
__global__ void mul_fp32(const float* a, const float* b, float* output,
                         int element_count) {
...
float benchmark_us(const float* a, const float* b, float* output,
  ...
  for (int i = 0; i < kWarmupIterations; ++i) {
    mul_fp32<<<blocks, kThreads>>>(a, b, output, element_count);
  }
...
void validate(int element_count, std::uint32_t seed) {
  ...
  mul_fp32<<<blocks_for(element_count), kThreads>>>(
  ...
  for (int i = 0; i < element_count; ++i) {
    const float reference = inputs.a[i] * inputs.b[i];
  ...
  std::cout << "Validation mismatches: " << mismatches << " / "
            << element_count << '\n';
  std::cout << "Max absolute error: " << max_absolute_error << '\n';
  std::cout << "Max relative error: " << max_relative_error << '\n';
  if (mismatches != 0) {
    throw std::runtime_error("FP32 multiply validation failed");
  }
  ...
  try {
    if (argc > 5) {
      throw std::invalid_argument(
          "usage: mul.exe [elements] [iterations] [validation_elements] "
          "[seed]");
    }
    ...
    const float elapsed_us = benchmark_us(
        a.get(), b.get(), output.get(), elements, iterations);
    ...
    std::cout << "Elementwise multiply: " << elapsed_us << " us\n";
