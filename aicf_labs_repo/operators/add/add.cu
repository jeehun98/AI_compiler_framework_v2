...
__global__ void add_fp32(const float* a, const float* b, float* output,
                         int element_count) {
...
float benchmark_us(const float* a, const float* b, float* output,
  ...
  for (int i = 0; i < kWarmupIterations; ++i) {
    add_fp32<<<blocks, kThreads>>>(a, b, output, element_count);
  }
...
void validate(int element_count, std::uint32_t seed) {
  ...
  add_fp32<<<blocks_for(element_count), kThreads>>>(
  ...
  if (mismatches != 0) {
    throw std::runtime_error("FP32 add validation failed");
  }
  ...
  try {
    if (argc > 5) {
      throw std::invalid_argument(
          "usage: add.exe [elements] [iterations] [validation_elements] "
          "[seed]");
    }
    ...
    std::cout << "Elementwise add: " << elapsed_us << " us\n";
