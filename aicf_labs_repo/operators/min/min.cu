...
__global__ void min_fp32(const float* a, const float* b, float* output,
                            int element_count) {
  const int index = blockIdx.x * blockDim.x + threadIdx.x;
  if (index < element_count) {
    output[index] = fminf(a[index], b[index]);
  }
}
...
void copy_inputs_to_device(const HostInputs& inputs, DeviceFloatBuffer& a,
                           DeviceFloatBuffer& b) {
...
float benchmark_us(const float* a, const float* b, float* output,
                   int element_count, int iterations) {
  const int warmup_iterations = iterations > 1 ? std::min(10, iterations) : 0;
  const int blocks = blocks_for(element_count);
  for (int i = 0; i < warmup_iterations; ++i) {
    min_fp32<<<blocks, kThreads>>>(a, b, output, element_count);
  }
...
void validate(int element_count, std::uint32_t seed) {
  ...
  copy_inputs_to_device(inputs, a, b);

  min_fp32<<<blocks_for(element_count), kThreads>>>(
      a.get(), b.get(), output.get(), element_count);
  CUDA_CHECK(cudaGetLastError());
  CUDA_CHECK(cudaDeviceSynchronize());
  ...
  for (int i = 0; i < element_count; ++i) {
    const float reference = std::fmin(inputs.a[i], inputs.b[i]);
  ...
  if (tolerance_violations != 0) {
    throw std::runtime_error("FP32 min validation failed");
  }
  ...
  try {
    if (argc > 5) {
      throw std::invalid_argument(
          "usage: min.exe [elements] [iterations] [validation_elements] "
          "[seed]");
    }
    ...
    const float elapsed_us =
        benchmark_us(a.get(), b.get(), output.get(), elements, iterations);
    ...
    std::cout << "Elementwise min: " << elapsed_us << " us\n";
