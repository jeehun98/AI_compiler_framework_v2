...
__global__ void sub_fp32(const float* a, const float* b, float* output,
                            int element_count) {
...
float benchmark_us(const float* a, const float* b, float* output,
  ...
  for (int i = 0; i < warmup_iterations; ++i) {
    sub_fp32<<<blocks, kThreads>>>(a, b, output, element_count);
  }
...
void validate(int element_count, std::uint32_t seed) {
  ...
  sub_fp32<<<blocks_for(element_count), kThreads>>>(
  ...
  if (tolerance_violations != 0) {
    throw std::runtime_error("FP32 sub validation failed");
  }
  ...
  try {
    if (argc > 5) {
      throw std::invalid_argument(
          "usage: sub.exe [elements] [iterations] [validation_elements] "
          "[seed]");
    }
    ...
    std::cout << "Elementwise sub: " << elapsed_us << " us\n";
